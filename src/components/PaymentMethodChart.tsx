...
export default function PaymentMethodChart({
  ...
  const data = useMemo<Slice[]>(() => {
    ...
    for (const t of transactions) {
      ...
      const pm = t.payment_method_id ? pmById[t.payment_method_id] : null;
      const key = pm?.id ?? 'sem-meio';
      const name = pm
        ? pm.name + (pm.last_four ? ` •••• ${pm.last_four}` : '')
        : 'Sem meio definido';
      const color = pm?.color ?? SEM_MEIO_COLOR;
      const current = totals.get(key);
      if (current) {
        current.value += Number(t.amount);
      } else {
        totals.set(key, { name, value: Number(t.amount), color });
      }
    }
    return [...totals.values()].sort((a, b) => b.value - a.value);
  }, [transactions, catById, pmById, kind]);
